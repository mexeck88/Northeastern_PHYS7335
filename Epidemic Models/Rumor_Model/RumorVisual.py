...
""" RumorVisual.py
Python module for simulating and visualizing the rumor model on networks.
This module generates various graph types and performs rumor epidemics on them.
...
# Default Parameters
NUM_NODES = 100
LAMBDA = 0.3  # Spreading rate (Ignorant -> Spreader)
ALPHA  = 0.1  # Stifling rate (Spreader -> Stifler)
NUM_STEPS = 100
INITIAL_INFECTED = 1

USAGE = r"""
    RumorVisual.py -n <Number of Nodes> -l <Spreading Rate Lambda> -a <Stifling Rate Alpha>

Flags (all optional; defaults shown in brackets):
    -n, --nodes  <Number of Nodes>      Number of nodes [default: 100]
    -l, --lamb   <Spreading Rate>       Spreading probability (I->S) [default: 0.3]
    -a, --alpha  <Stifling Rate>        Stifling probability (S->R) [default: 0.1]
...
def rumor_step(graph, states, lamb, alpha):
    """
    Perform a single step of the rumor spread model.
    States: 'I' = Ignorant, 'S' = Spreader, 'R' = Stifler
    Returns: new_states (dict)
    """
    new_states = states.copy()
    nodes = list(graph.nodes())
    
    # Identify current Spreader nodes
    spreader_nodes = [n for n in nodes if states[n] == 'S']
    
    # Rumor spreading: Spreader contacts Ignorant neighbor (S -> I contact -> I becomes S)
    for node in spreader_nodes:
        neighbors = list(graph.neighbors(node))
        for neighbor in neighbors:
            if states[neighbor] == 'I':
                # Ignorant adopts the rumor and becomes a Spreader
                if random.random() < lamb:
                    new_states[neighbor] = 'S'
    
    # Stifling: Spreader contacts another Spreader or Stifler and stops (S -> R)
    for node in spreader_nodes:
        # Uses OLD state so newly converted Spreaders this step are not stifled yet
...
def visualize_rumor(graph, graph_name, lamb, alpha, num_steps, F=0):
    """
    Perform rumor visualization for the passed graph.
    ...
    print(f"[INFO] Visualizing {graph_name} (λ={lamb}, α={alpha})")

    # Create frames directory
    frames_dir = "frames_rumor"
    ...
    # Initialize States
    states = {node: 'I' for node in graph.nodes()}
    ...
    states[central_node] = 'S'
    print(f"[INFO] Initial patient zero: {central_node}")
    
    # Colors suitable for S, I, R
    # I: Blue (Susceptible/calm)
    # S: Red (Infected/danger)
    # R: Green (Recovered/safe)
    color_map = {'I': '#1f77b4', 'S': '#d62728', 'R': '#2ca02c'}
    ...
    plt.title(f"{graph_name} - Rumor Spread Model - Step 0", fontsize=14, fontweight='bold', pad=10)
    
    s_count = list(states.values()).count('S')
    i_count = list(states.values()).count('I')
    r_count = list(states.values()).count('R')
    stats_text = f"I: {i_count} | S: {s_count} | R: {r_count}\nλ: {lamb} | α: {alpha}"
    ...
    # Simulation Loop
    for step in tqdm(range(num_steps), desc=f"Simulating rumor spread on {graph_name}"):
        states = rumor_step(graph, states, lamb, alpha)
        ...
        plt.title(f"{graph_name} - Rumor Spread Model - Step {step + 1}", fontsize=14, fontweight='bold', pad=10)
    ...
    gif_path = os.path.join(output_dir, f"{graph_name}_Rumor_animation.gif")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Rumor Spread Visualization", usage=USAGE, add_help=False)
    parser.add_argument("-n", "--nodes", type=int,   default=NUM_NODES, help="Number of nodes")
    parser.add_argument("-l", "--lamb",  type=float, default=LAMBDA,    help="Spreading rate (lambda)")
    parser.add_argument("-a", "--alpha", type=float, default=ALPHA,     help="Stifling rate (alpha)")
    ...
    return args.nodes, args.lamb, args.alpha, args.steps

def main():
    num_nodes, lamb, alpha, num_steps = parse_args()
    
    print(f"[INFO] Running rumor spread model with N={num_nodes}, λ={lamb}, α={alpha}")
    
    # Generate Graphs
    # 1. ER Graph
    er_graph = nx.erdos_renyi_graph(num_nodes, 0.5)
    visualize_rumor(er_graph, "ER_Graph", lamb, alpha, num_steps)
    
    # 2. WS Graph
    ws_graph = nx.watts_strogatz_graph(num_nodes, k=4, p=0.1)
    visualize_rumor(ws_graph, "WS_Graph", lamb, alpha, num_steps)
    
    # 3. BA Graph
    ba_graph = nx.barabasi_albert_graph(num_nodes, 2)
    visualize_rumor(ba_graph, "BA_Graph", lamb, alpha, num_steps)
    ...
    visualize_rumor(config_model, "Config_Model", lamb, alpha, num_steps)
